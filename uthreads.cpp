...
#define NUM_QUANT_ERROR_MSG "sleep need to be set to positive number of quantoms."
#define CANT_BLOCK_MAIN_ERR_MSG "Main thread can't be blocked."
/****************************** THREAD STRUCT *******************************/
...
int uthread_block(int tid)
{
    ...
    {
        unmask_timer();
        std::cerr << LIBRARY_ERROR_PREFIX << CANT_BLOCK_MAIN_ERR_MSG << std::endl;
        return FAILURE_RETURN_VALUE;
...
int uthread_sleep(int num_quantums)
{
    ...
    if(num_quantums < 0)
    {
        std::cerr << LIBRARY_ERROR_PREFIX << NUM_QUANT_ERROR_MSG << std::endl;
        unmask_timer();
        return FAILURE_RETURN_VALUE;
    ...
    if (did_just_save_bookmark)
    {
        threads[curr_thread_id]->sleeping_time = num_quantums+1;
